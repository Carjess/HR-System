<!-- 
    El 'aside' usa la variable 'isSidebarReady' (definida en app.blade.php)
    para saber si debe aplicar la transición o no. Esto evita el parpadeo al cargar.
-->
<aside :class="[
            sidebarOpen ? 'w-64' : 'w-20',
            isSidebarReady ? 'transition-all duration-300 ease-in-out' : ''
       ]" 
       class="flex flex-col flex-shrink-0 h-full bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 z-20">
    
    <!-- 1. CABECERA (LOGO Y BOTÓN) -->
    <div class="flex items-center h-16 px-4 border-b border-gray-200 dark:border-gray-700 overflow-hidden"
         :class="[
            sidebarOpen ? 'justify-between' : 'justify-center',
            isSidebarReady ? 'transition-all duration-300' : ''
         ]">
        
        <!-- Logo / Nombre (Usamos x-show para ocultarlo inmediatamente si la barra se cierra) -->
        ...
        <!-- Botón para Colapsar (Las 3 líneas) -->
    ...
    <div class="flex-1 flex flex-col overflow-y-auto px-3 py-4 space-y-2">
        ...
        @can('is-admin')
            <!-- AÑADIDO x-cloak AQUÍ -->
            <div x-show="sidebarOpen" x-cloak class="px-3 mt-6 mb-2 text-xs font-semibold text-gray-400 uppercase tracking-wider transition-opacity duration-200">
                Administración
            </div>

            <a href="{{ route('empleados.index') }}" class="flex items-center p-3 rounded-lg transition-colors duration-200 {{ request()->routeIs('empleados.*') ? 'bg-blue-50 text-blue-600 dark:bg-gray-700 dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900' }}">
            ...
            <a href="{{ route('tipos-contrato.index') }}" class="flex items-center p-3 rounded-lg transition-colors duration-200 {{ request()->routeIs('tipos-contrato.*') ? 'bg-blue-50 text-blue-600 dark:bg-gray-700 dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900' }}">
            ...
            <a href="{{ route('payroll.index') }}" class="flex items-center p-3 rounded-lg transition-colors duration-200 {{ request()->routeIs('payroll.*') ? 'bg-blue-50 text-blue-600 dark:bg-gray-700 dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900' }}">
            ...
            <a href="{{ route('ausencias.index') }}" class="flex items-center p-3 rounded-lg transition-colors duration-200 {{ request()->routeIs('ausencias.*') ? 'bg-blue-50 text-blue-600 dark:bg-gray-700 dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900' }}">
        ...
        @if(auth()->user()->role === 'employee')
             <!-- AÑADIDO x-cloak AQUÍ -->
             <div x-show="sidebarOpen" x-cloak class="px-3 mt-6 mb-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                Mi Espacio
            </div>
            
            <a href="{{ route('empleados.show', auth()->user()->id) }}" class="flex items-center p-3 rounded-lg transition-colors duration-200 {{ request()->routeIs('empleados.show') ? 'bg-blue-50 text-blue-600 dark:bg-gray-700 dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900' }}">
    ...
    <div class="p-4 border-t border-gray-200 dark:border-gray-700">
        <a href="{{ route('profile.edit') }}" class="flex items-center w-full text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors duration-200">
